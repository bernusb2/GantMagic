  ...
  private
    ...
    def generateHTML
      ...
      head = html.generateHead(@project['name'] + " - #{get('title') || @name}",
                               { 'description' => 'GantMagic Report',
                                 'keywords' =>
                                   'gantmagic, project, management, gantt chart' },
                               a('rawHtmlHead'))
      # Apply the user's saved theme preference (or the OS preference) as
      # early as possible to avoid a flash of the wrong theme.
      head << (themeScript = XMLElement.new('script', 'type' => 'text/javascript'))
      themeScript << XMLBlob.new(<<'EOT'

(function () {
  try {
    var theme = localStorage.getItem('tjTheme');
    if (!theme) {
...
EOT
      ...
      body << (themeToggle = XMLElement.new('button',
                                            'type' => 'button',
                                            'class' => 'tj_theme_toggle',
                                            'title' => 'Toggle dark/light theme',
                                            'aria-label' => 'Toggle dark/light theme',
                                            'onclick' => 'tjToggleTheme()'))
      themeToggle << XMLText.new("\u25D0")
      body << (toggleScript = XMLElement.new('script', 'type' => 'text/javascript'))
      toggleScript << XMLBlob.new(<<'EOT'

function tjToggleTheme() {
  var html = document.documentElement;
  var next = html.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
  html.setAttribute('data-theme', next);
  try { localStorage.setItem('tjTheme', next); } catch (e) {}
}
EOT
                                  )
...
improper GantMagic installation. If you know the directory, you can use the
GANTMAGIC_DATA_PATH environment variable to specify the location.  The
